...
class Route {
  ...
  private static $pathNotFound = ['ErrorHandle','handle404'];
  private static $methodNotAllowed = ['ErrorHandle','handle405'];
  ...
  public static function run($basepath = '/', $case_matters = false, $trailing_slash_matters = false, $multimatch = false) {
    ...
    if (!$route_match_found) {
      ...
      if ($path_match_found) {
        if (self::$methodNotAllowed) { // 405
          call_user_func_array(self::$methodNotAllowed, Array($path,$method));
        }
      } else {
        if (self::$pathNotFound) { // 404
